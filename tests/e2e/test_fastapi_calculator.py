...
def register_and_login(client, user_data: dict) -> dict:
    """
    ...
    reg_response = client.post(reg_url, json=user_data)
...
def test_user_login(client):
    ...
    # Register user
    reg_response = client.post(reg_url, json=test_user)
    assert reg_response.status_code == 201, f"User registration failed: {reg_response.text}"
...
def test_create_calculation_addition(client):
    user_data = {
        "first_name": "Calc",
        "last_name": "Adder",
...
def test_create_calculation_subtraction(client):
    user_data = {
        "first_name": "Calc",
...
def test_create_calculation_multiplication(client):
    user_data = {
        "first_name": "Calc",
...
def test_create_calculation_division(client):
    user_data = {
        "first_name": "Calc",
        "last_name": "Divider",
...
def test_list_get_update_delete_calculation(client):
    ...
    get_url = f"/calculations/{calc_id}"
    ...
    update_url = f"/calculations/{calc_id}"
    update_payload = {"inputs": [5, 6]}
    update_response = client.put(update_url, json=update_payload, headers=headers)
    ...
    # Delete calculation
    delete_url = f"/calculations/{calc_id}"
    ...
    get_response_after_delete = client.get(get_url, headers=headers)
